...
from gdt.missions.fermi.gbm.finders import TriggerFinder, ContinuousFinder  # 修正导入
...
class gbmRetrieve(Retrieve):
    ...
    @classmethod
    def from_burst_ftp(cls, burstid, datapath=None, skip_tte=False, skip_healpix=False):
        # 原有 from_burst 方法整体搬迁到这里，内容不变
        if datapath is None:
            datapath = '/home/xiniang/Data/fermi/data/gbm/bursts'
    ...
    def from_burst(cls, burstid, datapath=None, skip_tte=False, skip_healpix=False):
        """
        使用 gdt 的 TriggerFinder 下载数据，返回与原方法一致的 rtv_res 结构
        """
        if datapath is None:
            datapath = '/home/xiniang/Data/fermi/data/gbm/bursts'
        year = '20' + burstid[2:4]
        local_dir = os.path.join(datapath, year, burstid, 'current')
        if not os.path.isdir(local_dir):
            os.makedirs(local_dir)

        finder = TriggerFinder(burstid)
        tte_dict = {}
        if not skip_tte:
            # 默认下载全部探测器的TTE文件
            tte_paths = finder.get_tte(local_dir)
            # 解析探测器名并归类
            for p in tte_paths:
                ...
                if len(parts) > 2:
                    det = parts[2]
                    if det not in tte_dict:
                        tte_dict[det] = []
                    tte_dict[det].append(str(p))
            # 保证所有探测器键都存在
            all_dets = sorted(set([parts[2] for parts in (f.split('_') for f in finder.ls_tte()) if len(parts) > 2]))
            for det in all_dets:
                if det not in tte_dict:
                    tte_dict[det] = [None]
        else:
        ...
        if not skip_healpix:
            healpix_files = finder.get_healpix(download_dir=local_dir)
            healpix = str(healpix_files[0]) if healpix_files else None
        else:
            healpix = None
    ...
    @classmethod
    def from_utc_ftp(cls, utc, t1, t2, datapath=None, skip_tte=False, skip_poshist=False):
        # 原有 from_utc 方法整体搬迁到这里，内容不变
        if datapath is None:
            datapath = '/home/xiniang/Data/fermi/data/gbm/daily'
        ...
        if not skip_tte:
            for date in dates_perH:
                utcstr = date.strftime('%Y-%m-%dT%H:%M:%S')
                finder = ContinuousFinder(utcstr)
                for det in dets:
                    tte_file = finder.get_tte(det, download_dir=datapath)
                    tte_dict[det].append(tte_file if tte_file else None)
        else:
            for det in dets:
                tte_dict[det] = [None for _ in dates_perH]
        ...
        if not skip_poshist:
            for date in dates_perD:
                utcstr = date.strftime('%Y-%m-%dT%H:%M:%S')
                finder = ContinuousFinder(utcstr)
                poshist_file = finder.get_poshist(download_dir=datapath)
                poshist_list.append(poshist_file if poshist_file else None)
        else:
            poshist_list = [None for _ in dates_perD]
    ...
    def from_utc(cls, utc, t1, t2, datapath=None, skip_tte=False, skip_poshist=False):
        """
        使用 gdt 的 ContinuousFinder 下载数据，返回与原方法一致的 rtv_res 结构
        """
        import numpy as np
...
class gecamRetrieve(Retrieve):
    ...
    def from_burst(cls, burstid, datapath=None):
        ...
        if datapath is None:
            datapath = '/home/xiniang/Data/gecam/bursts'
    ...
    def from_utc(cls, utc, t1, t2, datapath=None):
        ...
        if datapath is None:
            datapath = '/home/xiniang/Data/gecam/daily'
...
class gridRetrieve(Retrieve):
    ...
    def from_utc(cls, utc, t1, t2, det, datapath=None):
        ...
        if datapath is None:
            datapath = '/home/xiniang/Data/grid/G05'
...
class epRetrieve(Retrieve):
    ...
    def from_wxtobs(cls, obsid, srcid, datapath=None): 
        ...
        if datapath is None:
            datapath = '/home/xiniang/Data/ep/WXT'
    ...
    def from_fxtobs(cls, obsid, module, datapath=None): 
        ...
        if datapath is None:
            datapath = '/home/xiniang/Data/ep/FXT'
